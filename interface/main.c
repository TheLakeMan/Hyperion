...
#include "web_server.h" // Include web server module
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
");
    ...
    printf("  -p, --port <number>     Port number for web server (default: 8080)\n");
    printf("  -w, --webdir <dir>      Web UI directory (default: web_ui)\n");
    printf("\n");
    printf("Commands:\n");
    ...
    printf("  serve                   Start web server interface\n");
    printf("\n");
}
...
static int cmdGenerate(picolInterp *interp, const char *modelPath, const char *tokenizerPath,
                       const char *prompt)
{
...
int main(int argc, char **argv)
{
    ...
    const char *webPort       = "8080";   // Default web server port
    const char *webDir        = "web_ui"; // Default web UI directory
    ...
    for (i = 1; i < argc; i++) {
        if (argv[i][0] == '-') {
            ...
            else if (strcmp(argv[i], "-w") == 0 || strcmp(argv[i], "--webdir") == 0) {
                if (i + 1 < argc) {
                    webDir = argv[++i];
                }
                else {
    ...
    if (!command) {
        ...
        /* Start web server */
        printf("Starting web server on port %s, serving UI from '%s'...\n", webPort, webDir);
        result = start_web_server(interp, webPort, webDir);
    }
    else {
        /* Unknown command */
