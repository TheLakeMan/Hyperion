...
static void test_tensor_creation()
{
    ...
    size_t            dims[] = {2, 3};
    HyperionTensorShape shape  = {.dims = dims, .num_dims = 2, .total_size = 6};

    // Test static memory allocation
    HyperionMemoryEfficientTensor *tensor =
        hyperionCreateMemoryEfficientTensor(&shape, TINYAI_TENSOR_FLOAT32, TINYAI_MEMORY_STATIC);
    assert(tensor != NULL);
    ...
    hyperionFreeMemoryEfficientTensor(tensor);

    // Test pooled memory allocation
    tensor = hyperionCreateMemoryEfficientTensor(&shape, TINYAI_TENSOR_FLOAT32, TINYAI_MEMORY_POOLED);
    assert(tensor != NULL);
    ...
    tensor = hyperionCreateMemoryEfficientTensor(&shape, TINYAI_TENSOR_FLOAT32, TINYAI_MEMORY_STREAM);
    assert(tensor != NULL);
...
static void test_inplace_operations()
{
    ...
    size_t            dims[] = {2, 2};
    HyperionTensorShape shape  = {.dims = dims, .num_dims = 2, .total_size = 4};

    // Create tensors
    HyperionMemoryEfficientTensor *tensor1 =
        hyperionCreateMemoryEfficientTensor(&shape, TINYAI_TENSOR_FLOAT32, TINYAI_MEMORY_STATIC);
    HyperionMemoryEfficientTensor *tensor2 =
        hyperionCreateMemoryEfficientTensor(&shape, TINYAI_TENSOR_FLOAT32, TINYAI_MEMORY_STATIC);
    ...
    assert(hyperionSetTensorData(tensor1, data1, sizeof(data1)));
    assert(hyperionSetTensorData(tensor2, data2, sizeof(data2)));

    // Test addition
    assert(hyperionTensorAddInPlace(tensor1, tensor2));
    float *result = (float *)hyperionGetTensorData(tensor1);
    ...
    // Test multiplication
    assert(hyperionTensorMulInPlace(tensor1, tensor2));
    result = (float *)hyperionGetTensorData(tensor1);
    assert(result[0] == 0.75f);
    assert(result[1] == 5.25f);
    assert(result[2] == 13.75f);
    assert(result[3] == 26.25f);

    hyperionFreeMemoryEfficientTensor(tensor1);
    hyperionFreeMemoryEfficientTensor(tensor2);
}
...
static void test_streaming_operations()
{
    // Create a 4x4 tensor
    size_t            dims[] = {4, 4};
    HyperionTensorShape shape  = {.dims = dims, .num_dims = 2, .total_size = 16};
    ...
    assert(hyperionTensorStreamOperation(tensor1, tensor2, add_operation, 4));
    float *result = (float *)hyperionGetTensorData(tensor1);
    for (int i = 0; i < 16; i++) {
...
static void test_memory_pool()
{
    ...
    size_t            dims[] = {3, 3};
    HyperionTensorShape shape  = {.dims = dims, .num_dims = 2, .total_size = 9};

    HyperionMemoryEfficientTensor *tensor =
        hyperionCreateMemoryEfficientTensor(&shape, TINYAI_TENSOR_FLOAT32, TINYAI_MEMORY_POOLED);
    assert(tensor != NULL);

    // Test memory allocation from pool
    void *ptr1 = hyperionTensorPoolAlloc(tensor, sizeof(float) * 4);
    assert(ptr1 != NULL);

    void *ptr2 = hyperionTensorPoolAlloc(tensor, sizeof(float) * 3);
    assert(ptr2 != NULL);

    // Test memory freeing
    hyperionTensorPoolFree(tensor, ptr1);
    hyperionTensorPoolFree(tensor, ptr2);

    // Test pool resizing
    assert(hyperionResizeTensorPool(tensor, tensor->pool_size * 2));
...
static void test_memory_optimization()
{
    ...
    // Test memory optimization
    assert(hyperionOptimizeTensorMemory(tensor));
    assert(tensor->is_contiguous == true);
