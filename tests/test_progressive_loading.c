...
{
    HyperionProgressiveLoaderConfig config = {.max_memory_budget      = TEST_MEMORY_BUDGET,
                                            .enable_layer_unloading = true,
                                            .priority_strategy  = TINYAI_PRIORITY_ACCESS_PATTERN,
                                            .prefetch_threshold = 0.7f};

    HyperionProgressiveLoader *loader = hyperionCreateProgressiveLoader(TEST_MODEL_PATH, &config);
    assert(loader != NULL);
...
static void test_layer_management(void)
{
    ...
    for (int i = 0; i < TEST_LAYER_COUNT; i++) {
        assert(hyperionLoadModelLayer(loader, i) == true);
        assert(hyperionIsLayerLoaded(loader, i) == true);
    }

    // Test unloading layers
    for (int i = 0; i < TEST_LAYER_COUNT; i++) {
        assert(hyperionUnloadModelLayer(loader, i) == true);
        assert(hyperionIsLayerLoaded(loader, i) == false);
    }
...
static void test_memory_budget(void)
{
    ...
    for (int i = 0; i < TEST_LAYER_COUNT; i++) {
        if (hyperionLoadModelLayer(loader, i)) {
            ...
            if (total_memory > TEST_MEMORY_BUDGET) {
                ...
                assert(hyperionGetProgressiveLoaderMemoryStats(loader).total_allocated <=
                       TEST_MEMORY_BUDGET);
                break;
            }
        }
...
static void test_layer_dependencies(void)
{
    ...
    assert(hyperionAddLayerDependency(loader, 1, 0) == true);

    // Try to unload layer 0 while layer 1 is loaded
    assert(hyperionLoadModelLayer(loader, 1) == true);
    assert(hyperionUnloadModelLayer(loader, 0) == false); // Should fail due to dependency

    // Unload layer 1 first, then layer 0
    assert(hyperionUnloadModelLayer(loader, 1) == true);
    assert(hyperionUnloadModelLayer(loader, 0) == true);
...
static void test_access_patterns(void)
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        hyperionUpdateLayerAccessStats(loader, layer);
    }
...
static void test_prefetching(void)
{
    ...
    // Load current layer
    assert(hyperionLoadModelLayer(loader, 0) == true);

    // Get prefetch recommendations
    int  prefetch_count;
    int *prefetch_layers = hyperionGetLayersToPreload(loader, 0, &prefetch_count);
...
static void benchmark_progressive_loading(void)
{
    ...
    HyperionBenchmarkOperation op = {.setup     = NULL,
                                   .operation = (void (*)(void *))hyperionLoadModelLayer,
                                   .teardown  = NULL,
                                   .context   = loader};

    HyperionBenchmarkResult result = hyperionBenchmarkOperation("Layer Loading", 100, op);
