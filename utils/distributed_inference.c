...
static void freeModelPartition(HyperionModelPartition *partition) {
    if (!partition) return;

    hyperionFree(partition->partitionSizes);
    hyperionFree(partition->nodeAssignments);

    if (partition->partitionData) {
        for (int i = 0; i < partition->numPartitions; i++) {
            hyperionFree(partition->partitionData[i]);
        }
        hyperionFree(partition->partitionData);
    }

    hyperionFree(partition->layerToStage);
    hyperionFree(partition->tensorSplitDims);
...
void hyperionDistributedClusterFree(HyperionDistributedCluster *cluster) {
    ...
    if (cluster->modelPartition) {
        freeModelPartition(cluster->modelPartition);
        cluster->modelPartition = NULL;
    }
...
bool hyperionDistributedDeployModel(HyperionDistributedCluster *cluster,
    ...
    if (!cluster || !partition) return false;

#ifndef _WIN32
    pthread_mutex_lock(&cluster->clusterMutex);
#endif

    if (partition->numPartitions > 0 && partition->nodeAssignments) {
        for (int i = 0; i < partition->numPartitions; i++) {
            int nodeIndex = partition->nodeAssignments[i];
            if (nodeIndex < 0 || nodeIndex >= cluster->nodeCount) {
#ifndef _WIN32
                pthread_mutex_unlock(&cluster->clusterMutex);
#endif
                return false;
    ...
    if (cluster->modelPartition && cluster->modelPartition != partition) {
        freeModelPartition(cluster->modelPartition);
    }

    cluster->modelPartition = (HyperionModelPartition*)partition;
    cluster->running = true;

    if (partition->numPartitions > 0 && partition->nodeAssignments) {
        for (int i = 0; i < partition->numPartitions; i++) {
            int nodeIndex = partition->nodeAssignments[i];
            if (cluster->nodes[nodeIndex].status != HYPERION_NODE_FAILED) {
                cluster->nodes[nodeIndex].status = HYPERION_NODE_BUSY;
            }
        }
    }
...
int hyperionDistributedBatchInference(HyperionDistributedCluster *cluster,
    ...
    if (!cluster || !batchParams || !results || batchSize <= 0 || maxTokensPerItem <= 0) {
        return -1;
    }
    ...
    for (int i = 0; i < batchSize; i++) {
        ...
        int generated = hyperionDistributedGenerateText(cluster, &batchParams[i],
                                                        results[i], maxTokensPerItem);
        if (generated < 0) {
            continue;
        }
        for (int t = generated; t < maxTokensPerItem; t++) {
            results[i][t] = -1;
        }
        processed++;
    }
...
bool hyperionDistributedRemoveNode(HyperionDistributedCluster *cluster, int nodeIndex) {
    ...
    if (cluster->workerSockets[nodeIndex] >= 0) {
        close(cluster->workerSockets[nodeIndex]);
    }

    for (int i = nodeIndex; i < cluster->nodeCount - 1; i++) {
        cluster->nodes[i] = cluster->nodes[i + 1];
        cluster->workerSockets[i] = cluster->workerSockets[i + 1];
    }

    cluster->workerSockets[cluster->nodeCount - 1] = -1;
    cluster->nodeCount--;

    if (cluster->localNodeIndex == nodeIndex) {
        cluster->localNodeIndex = -1;
    } else if (cluster->localNodeIndex > nodeIndex) {
        cluster->localNodeIndex--;
    }
...
bool hyperionDistributedSetFaultTolerance(HyperionDistributedCluster *cluster,
    ...
    if (!cluster) return false;
    cluster->config.faultTolerance = mode;
    return true;
}

bool hyperionDistributedCreateCheckpoint(HyperionDistributedCluster *cluster,
                                         const char *checkpointPath) {
    if (!cluster || !checkpointPath || !cluster->config.enableCheckpointing) {
        return false;
    }

    FILE *fp = fopen(checkpointPath, "wb");
    if (!fp) {
        return false;
    }

    if (fwrite(&cluster->nodeCount, sizeof(int), 1, fp) != 1 ||
        fwrite(&cluster->totalTasksProcessed, sizeof(uint64_t), 1, fp) != 1 ||
        fwrite(&cluster->config, sizeof(HyperionDistributedConfig), 1, fp) != 1) {
        fclose(fp);
        return false;
    }

    for (int i = 0; i < cluster->nodeCount; i++) {
        if (fwrite(&cluster->nodes[i], sizeof(HyperionNodeInfo), 1, fp) != 1) {
            fclose(fp);
            return false;
        }
    }
...
bool hyperionDistributedRestoreCheckpoint(HyperionDistributedCluster *cluster,
                                          const char *checkpointPath) {
    if (!cluster || !checkpointPath) return false;

    FILE *fp = fopen(checkpointPath, "rb");
    if (!fp) {
        return false;
    }
    ...
    HyperionDistributedConfig savedConfig;

    if (fread(&nodeCount, sizeof(int), 1, fp) != 1 ||
        fread(&totalTasks, sizeof(uint64_t), 1, fp) != 1 ||
        fread(&savedConfig, sizeof(HyperionDistributedConfig), 1, fp) != 1) {
        fclose(fp);
        return false;
    }

    if (nodeCount > cluster->config.maxNodes) {
        nodeCount = cluster->config.maxNodes;
    }
    ...
    cluster->nodeCount = 0;
    cluster->totalTasksProcessed = totalTasks;
    cluster->config = savedConfig;

    for (int i = 0; i < nodeCount; i++) {
        if (fread(&cluster->nodes[i], sizeof(HyperionNodeInfo), 1, fp) != 1) {
#ifndef _WIN32
            pthread_mutex_unlock(&cluster->clusterMutex);
#endif
            fclose(fp);
...
bool hyperionDistributedSetLoadBalancing(HyperionDistributedCluster *cluster,
    ...
    if (!cluster) return false;
    cluster->config.loadBalanceStrategy = strategy;
    return true;
}

bool hyperionDistributedMonitorHealth(const HyperionDistributedCluster *cluster,
                                      char *healthReport, size_t reportSize) {
    if (!cluster || !healthReport || reportSize == 0) return false;

    int activeNodes = 0;
    for (int i = 0; i < cluster->nodeCount; i++) {
        if (cluster->nodes[i].status == HYPERION_NODE_ACTIVE ||
            cluster->nodes[i].status == HYPERION_NODE_BUSY) {
            activeNodes++;
        }
    }

    int offset = snprintf(healthReport, reportSize,
                          "{\"nodes\":%d,\"active\":%d,\"tasks\":%llu,\"avgLatency\":%.3f,\"details\":[",
                          cluster->nodeCount,
                          activeNodes,
                          (unsigned long long)cluster->totalTasksProcessed,
                          cluster->avgTaskLatency);
    ...
    for (int i = 0; i < cluster->nodeCount; i++) {
        const HyperionNodeInfo *node = &cluster->nodes[i];
        int written = snprintf(healthReport + offset, reportSize - (size_t)offset,
                               "{\"id\":\"%s\",\"status\":%d,\"load\":%.3f,\"memory\":%zu,\"latency\":%.3f}%s",
                               node->nodeId,
                               node->status,
                               node->currentLoad,
                               node->memoryUsed,
                               node->avgLatency,
                               (i == cluster->nodeCount - 1) ? "" : ",");
    ...
    int tail = snprintf(healthReport + offset, reportSize - (size_t)offset, "]}");
