...
static const HyperionMemoryAnalysisConfig DEFAULT_CONFIG = {.track_allocations   = true,
...
{
    HyperionMemoryAnalysis *analysis = malloc(sizeof(HyperionMemoryAnalysis));
    if (!analysis)
        return NULL;
    ...
    analysis->allocations     = malloc(sizeof(HyperionMemoryAllocation) * analysis->max_allocations);
    ...
    memset(&analysis->pattern, 0, sizeof(HyperionMemoryPattern));
                            ...
                            const char *file, int line, const char *function)
{
    ...
    if (analysis->num_allocations >= analysis->max_allocations) {
        ...
        HyperionMemoryAllocation *new_allocations =
            realloc(analysis->allocations, sizeof(HyperionMemoryAllocation) * new_size);
        if (!new_allocations)
            return;
    ...
    HyperionMemoryAllocation *alloc = &analysis->allocations[analysis->num_allocations++];
...
void hyperionRecordDeallocation(HyperionMemoryAnalysis *analysis, void *address)
{
    ...
    for (size_t i = 0; i < analysis->num_allocations; i++) {
        HyperionMemoryAllocation *alloc = &analysis->allocations[i];
                                 ...
                                 HyperionMemoryAllocation **hotspots, size_t *num_hotspots)
{
    ...
    HyperionMemoryAllocation *sorted =
        malloc(sizeof(HyperionMemoryAllocation) * analysis->num_allocations);
    if (!sorted)
        return;

    memcpy(sorted, analysis->allocations,
           sizeof(HyperionMemoryAllocation) * analysis->num_allocations);
    qsort(sorted, analysis->num_allocations, sizeof(HyperionMemoryAllocation),
    ...
    *hotspots     = malloc(sizeof(HyperionMemoryAllocation) * (*num_hotspots));
    if (*hotspots) {
        memcpy(*hotspots, sorted, sizeof(HyperionMemoryAllocation) * (*num_hotspots));
    }
                                   ...
                                   HyperionMemoryAllocation **leaks, size_t *num_leaks)
{
    ...
    *num_leaks = count;
    *leaks     = malloc(sizeof(HyperionMemoryAllocation) * count);
    if (*leaks) {
        size_t idx = 0;
        for (size_t i = 0; i < analysis->num_allocations; i++) {
            if (!analysis->allocations[i].is_freed) {
...
static int compare_allocations(const HyperionMemoryAllocation *a, const HyperionMemoryAllocation *b)
{
